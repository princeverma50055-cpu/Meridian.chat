...
import type { AIProvider, ChatTurn, GenerateOptions, GenerateResult } from '@/lib/ai/provider';
 ...
 * prompt as separate config rather than a message in the list.
 */
  ...
  async embeddings(): Promise<number[][]> {
    throw new Error(
      'Embeddings are not wired up through the Gemini chat provider. Meridian uses Voyage AI ' +
        'for embeddings regardless of chat provider — set VOYAGE_API_KEY (see lib/embeddings/provider.ts).'
    );
  },
